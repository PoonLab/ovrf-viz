...
#overlaps <- read.csv('overlaps_with_noise_50.csv', colClasses='character')
...
# Remove +0 frame shifts
non_splicing_overlaps <- non_splicing_overlaps %>% filter(shift != "+0")
...
#orfs <- read.csv('total_orfs_50.csv') # with noise
virus <- read.csv('try_baltimore_class.csv')
colnames(virus)[2] <- "Accession"
...
index <- match(virus$Accession, noverlaps$accn)
...
prot_index <- match(nprot$accno, virus$Accession)
virus_combined <- virus %>% left_join(nprot, by=c("Accession" = "accno"))
plot(virus$Length, virus$freq, log='x')

virus_combined$Proteins<- virus_combined$freq
write.csv(virus_combined, "no_cero_out_virus_df_inspection.csv")
#write.csv(virus_combined, "50_NOISE_no_cero_out_virus_df_inspection.csv")
